...
function Scenario({ mission }: { mission: Mission }) {
  ...
  return <div className="fp-lab"><header className="fp-top"><Link href="/">← Learning library</Link><span>MISSION {String(mission.id).padStart(2, "0")} / {missions.length + 1}</span><Link href="/#course=filepilot&chapter=1">FilePilot case study</Link></header><main>
    ...
    <section className="fp-card" aria-label="Active mission step"><p className="fp-eyebrow">{state.complete ? "DEBRIEF" : state.blocked ? "SAFETY BOUNDARY" : "YOUR NEXT DECISION"}</p><div aria-live="polite" aria-atomic="true"><h2>{state.title}</h2><p>{state.explanation}</p></div>
      ...
      {state.complete && <div className="fp-status"><strong>Mission {mission.id} debrief.</strong> Explain the outcome in your own words: what was permitted, what was rejected, and which evidence justified the decision?{mission.comparison && <div className="fp-comparison"><h3>Compare the alternatives</h3>{mission.comparison.map(c => <div key={c.title}><h4>{c.title}</h4><p>{c.explanation}</p></div>)}<p className="fp-small">Restart to try the other strategy. These are hypothetical alternatives, not additional actions you have executed.</p></div>}{mission.id < missions.length + 1 && <p><a href={`#mission=${mission.id + 1}`}>Continue to mission {mission.id + 1} →</a></p>}</div>}<button onClick={restart}>Restart this mission</button></section>
