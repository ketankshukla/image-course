...
const { hospitalMissions: missions } = loadTs(path.resolve(__dirname, '../lib/hospital-missions.ts'));
const { advanceMission } = loadTs(path.resolve(__dirname, '../lib/mission-types.ts'));
assert.deepEqual(missions.map(m => m.id), Array.from({length:30}, (_,i)=>i+1));
let total = 0;
for (const mission of missions) {
  assert.match(mission.boundary, /fictional teaching policies/);
  assert.match(mission.boundary, /No real patients/);
...
console.log(`PASS: Hospital ${missions.length} missions, ${total} states, all branches reachable and recoverable; completion cannot be skipped from start.`);
